...
<!-- Dropdown menu -->
<div id="dropdown" class="z-10 hidden divide-y divide-gray-100 rounded-lg shadow w-44 bg-zinc-900">
  <ul class="py-2 text-sm text-gray-200" aria-labelledby="dropdownDefaultButton">
    <li v-for="(value, index) in modelsArray" :key="index">
      <a href="#" class="block px-4 py-2  hover:bg-gray-600 hover:text-white">{{ value }}</a>
    </li>
  </ul>
</div>
...
socket.addEventListener("message", (event) => {
  ...
  switch (parsed.type) {
    case "getModels":
      modelsArray.value = parsed.data.models.split(",");
